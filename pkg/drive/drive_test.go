package drive_test

import (
	"context"
	"fmt"
	"log"
	"os"
	"strings"
	"testing"

	"github.com/gowsp/cloud189/internal/invoker"
	"github.com/gowsp/cloud189/internal/util"
	"github.com/gowsp/cloud189/pkg/app"
	"github.com/gowsp/cloud189/pkg/drive"
)

func init() {
	// os.Setenv("189_MODE", "1")
	os.Setenv("189_MODE", "0")
}

func TestDrive(t *testing.T) {
	api, _ := app.Open(invoker.DefaultPath())
	f := api.Personal()
	info, err := f.Stat("demo/1")
	if err != nil {
		return
	}
	f.Stat("demo/2")
	f.Stat("demo/2")
	fmt.Println(info)
}
func TestWalk(t *testing.T) {
	api, _ := app.Open(invoker.DefaultPath())
	f := api.Personal()
	f.ReadDir("demo")
	f.ReadDir("demo")
}
func TestDecode(t *testing.T) {
	data := ""
	v := util.DecryptAES([]byte("C8CAB983D32137EE5F076F204B21BBCD"[0:16]), strings.ToLower(data))
	fmt.Println(v)
}
func TestDelete(t *testing.T) {
	api, _ := app.Open(invoker.DefaultPath())
	f := api.Personal()
	err := f.RemoveAll(context.Background(), "demo1")
	fmt.Println(err)
}
func TestMakeDir(t *testing.T) {
	api, _ := app.Open(invoker.DefaultPath())
	f := api.Personal()
	err := f.Mkdir(context.Background(), "demo1", 0755)
	fmt.Println(err)
}
func TestUpload(t *testing.T) {
	api, _ := app.Open(invoker.DefaultPath())
	fs := api.Personal()
	_ = drive.PutOptions{}
	// fs.Upload(cfg, "/home", "D:/repo/go/src/github.com/gowsp/cloud189/docs/html")
	// fs.Upload(cfg, "/", "D:/tmp/01.txt")
	_ = fs
	// f, _ := os.Open("drive_test.go")
	// l := file.NewLocalFile("-11", f)
	// api.Uploader().Write(l)
	// api.Uploader().Write(file.NewNetFile("-11", "demo.txt", 111))
	// api.Uploader().Write(file.NewFastFile("-11", "fast://13598043FF94C80EFC76642E10D2121C:1372/demo_file.txt"))
}

func TestDownload(t *testing.T) {
	api, _ := app.Open(invoker.DefaultPath())
	f := api.Personal()
	d, err := os.CreateTemp("", "cloud189-test-download-*")
	if err == nil {
		defer os.Remove(d.Name())
		defer d.Close()
		_, err = f.Download(context.Background(), "demo/page.html", d)
	}
	if err != nil {
		log.Println(err)
	}
}
