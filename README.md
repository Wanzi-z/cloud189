...
个人云路径保持以`/`开头。使用`-f`/`--family {家庭ID}`后，普通`/路径`表示该家庭云；`{家庭ID}:/路径`仅用于跨空间转存或脚本。使用`cloud189 fam`查看家庭 ID，例如：`cloud189 -f 123456789 ls /`

全局参数`-j`/`--json`以 JSON 格式输出结果，适用于脚本解析和自动化场景，支持以下命令: `df`、`stat`、`ls`、`dl`、`up`
...
- 退出登录
  ...
  - `cloud189 logout -y` 或 `cloud189 logout --force` 不询问直接退出
- 家庭云列表: `cloud189 fam` 显示家庭 ID、备注名、成员数和角色（兼容别名：`families`）
- 跨空间转存:
  - 个人云转家庭云: `cloud189 cp /文件 123456789:/目标目录`
  - 家庭云转个人云: `cloud189 cp 123456789:/文件 /目标目录`
  - 暂不支持家庭云之间转存
...
- 文件上传: `cloud189 up  -p {上传并发数默认5} -n {文件过滤表达式, 不匹配则不上传, 默认不过滤} {本地路径|http~~|fast...~~} {云盘路径}`，支持三种模式文件上传，已知网页版上传接口存在bug不支持断点续传, 例
  ...
  - 手动秒传 `cloud189 up {fast://文件MD5:文件大小/文件名...} {云盘路径}`，例 `cloud189 up fast://3BACAB45A36BE381390035D228BB23E0:7598080/cloud189 /我的应用`；大于 10MiB 时使用 `fast://文件MD5:sliceMD5:文件大小/文件名`。服务端未命中秒传时直接报错，不会创建空文件
  - 单文件上传`cloud189 up --input {本地路径} --path {云盘完整文件路径} --policy {skip|overwrite}`，支持指定完整远程文件路径和同名文件处理策略
- 文件下载: `cloud189 dl {云端路径...} {本地路径}` 支持文件夹, 支持断点续传。单文件下载可使用`-o {本地路径}`指定输出文件
- 文件列表: `cloud189 ls [-R] {云盘路径}` 大小为`-`表示文件夹, `-R` 递归列出子目录
- 文件信息: `cloud189 stat {云盘路径}` 查看文件或文件夹的详细信息
- 文件统计: `cloud189 du {云盘路径}` 显示文件或文件夹的使用统计信息，如果是文件夹则列出其中每个文件的统计信息
- 文件删除: `cloud189 rm {云盘路径...}`
- 文件移动: `cloud189 mv {云盘路径...} {目标路径}`
- 文件复制: `cloud189 cp {云盘路径...} {目标路径}`
